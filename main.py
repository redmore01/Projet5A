...
class Catalog(BoxLayout):
    ...
    # def view_try(self, **kwargs):
        # kwargs['cols'] = 3
        # super(Catalog, self).__init__(**kwargs)
		
        # list_image = lambda row, rec: {'text': rec['name'],
                                        # 'size_hint_y': None,
                                        # 'height': 25}
        # dict_adapter = \
                # DictAdapter(
        ...
        # list_view2 = ListView(adapter=dict_adapter2,
                                     # size_hint=(.2, 1.0))

        # dict_adapter.bind(
                # on_selection_change=dict_adapter2.fruits_changed)
